...
- [ ] Supports `protovalidate` rules（`proto-gen-validate` version >= 1.0）
...
```
However, in order to use the full functionality of `protobuf-to-pydantic`, can use the following command to install `protobuf-to-pydantic`:
```bash
...
```
#### 1.1.1.Use plugins
The plug-in is the `Pydantic Model` source code generation method recommended by `protobuf-to-pydantic.
...
```
Then after installing `protobuf-to-pydantic` can use the `--protobuf-to-pydantic_out` option to use the `protobuf-to-pydantic` plug-in, the command is as follows：
```bash
...
And it is declared that the output location of the `protobuf-to-pydantic` plug-in is `.` (`.` indicates the output path used by `grpc_tools.proto`).

After running the command, the `protobuf-to-pydantic` plug-in will write its generated content in the corresponding file.。
By default, the generated file name suffix is `p2p.py`, such as the code file generated by `protobuf-to-pydantic` for `example.proto` is named `example_p2p.py`
...
```
Next, in order to read this file, need to change `--protobuf-to-pydantic_out=.` in the command to `--protobuf-to-pydantic_out=config_path=plugin_config.py:.`,
Among them, the `config_path=plugin_config.py` on the left side of `:` indicates that the path of the configuration file to be read is `plugin_config.py`, and the right side of `:` is the same as the previous `.`, declaring `protobuf- The output location of the to-pydantic` plugin，
...
In `Python` runtime, The method `msg_to_pydantic_model` can be called to read the `UserMessage` object from the `demo_pb2` module and generate the corresponding `Pydantic Model` object as follows:
```Python
...
The `msg_to_pydantic_model` method is customizable just like plugins, with the following extension parameters:
...
The `pydantic_model_to_py_code` method is used to generate the source code, and the `pydantic_model_to_py_file` method is used to generate the code file. The example code of the `pydantic_model_to_py_file` method is as follows:
...
With these rules, the `Pydantic Model` object generated by `protobuf-to-pydantic` will have parameter checking.
Among them, text comments and P2P rules are consistent, they both support most of the parameters in `Pydantic Field`, some of the variations and new parameters are seen
[2.4.`P2P` and text comment rule other parameter support](#24p2p-and-text-comment-rule-other-parameter-support)

> NOTE:
>  - 1.Text comment rules are not the focus of subsequent functional iterative development, and it is recommended to use P2P verification rules.
>  - 2.`Protoc Plug-in` only support `PGV` and `P2P` rule.


### 2.1.Text comments
In the Protobuf file, can write comments for each field that meet the requirements of `protobuf-to-pydantic`,
...
Using `PGV` checksum rules in `protobuf-to-pydantic` is very simple, just write the corresponding `PGV` rules in the Protobuf file first, and then specify the value of `parse_msg_desc_method` to be `PGV` when calling `msg_to_pydantic_model` as the code below:
```Python
# pydantic version V1
from typing import Type
from protobuf_to_pydantic import msg_to_pydantic_model
...
### 2.3.P2P
The `PGV` checksum rules are written in the Option attribute of each field of `Message` and have a better code specification, so Protobuf that use `PGV` checksum rules will be more readable than Protobuf that use comments.
...
The `P2P` checksum rule that comes with `protobuf-to-pydantic` expands on the `PGV` checksum rule by incorporating some of the functionality of the text annotation checksum rule, which satisfies most of the customization of the properties of each `Field` in the `Pydantic Model`, such as the following Protobuf file.
```protobuf
syntax = "proto3";
...
### 2.4.`P2P` and text comment rule other parameter support
The `protobuf-to-pydantic` text comment rules and the `P2P` rules support most of the parameters in `FieldInfo`, as described in the [Pydantic Field doc](https://docs.pydantic.dev/latest/usage/fields/)。
...
|------------------|---------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| required         | False         | By default, the default value of each field in the generated `Pydantic Model` object is the same as the zero value of its corresponding type, although the setting of the default value is canceled when `required` is `True`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| const            | None          | Used to specify a constant value for a field, though different `Pydantic` versions behave differently<br/>  For `Pydantic V1`, the value of `default` in `Field` is set to the value specified by `const`, and `const` in `Field` is set to True.Note: `Pydantic Model`'s const only supports bool variables, when `const` is `True`, the accepted value can only be the value set by `default`, and the default value carried by the message generated by protobuf is the null value of the corresponding type does not match with `Pydantic Model`, so ` protobuf-to-pydantic` makes some changes to the input of this value.<br/> For `Pydantic V2`, the value of `default` in `Field` remains the same, but the type annotation changes to `typing_extensions.Literal[xxx]` |
| type             | None          | By default, the default type of a field is the same as Protobuf's, but use the [2.5.template](#25 template) function to modify the type of a field.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| extra            | None          | The `extra` parameter accepted by `Pydantic` is of type `Python Dict`, which is not supported by Protobuf, and requires the use of either [2.5. template] (#25 template) or the corresponding Json structure `protobuf-to-pydantic` in the Protobuf file to parse it properly.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| field            | None          | By default, the `Field` of the parameter is `Pydantic FieldInfo`, although it can be customized using the [2.5.Templates](#25 Templates) function                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
There is also support for fast import of `Pydantic type` for string types. For example, if you want to add a check for card numbers via the `pydantic.types.PaymentCardNumber` type, you can specify the type of the `pydantic_type` parameter field to be `PaymentCardNumber`, which is similar to the use of template imports in the `type` rule, as follows:
- Text comment rules：
  ```protobuf
  syntax = "proto3";
...
### 2.5.Template
When working with definition fields, you will find that some fields are filled with values that are methods or functions of one of the libraries in `Python` (e.g., the values of the `type` parameter and the `default_factory` parameter), which can't be accomplished with the Json syntax.
...
- Examples of text comment rules：
  ```protobuf
  syntax = "proto3";
...
- Examples of P2P rules：
  ...
  ```
However, the `msg_to_pydantic_model` method needs to be called with the parameter `local_dict` to register the corresponding value, the pseudo-code is as follows:
```Python
# a.py
import time
...
This template (which can be thought of as a simplified version of the `p2p@local` template) can be used directly when the variables to be used come from `Python` built-in functions, and the syntax is used as follows:
- Examples of text comment rules:
  ```protobuf
  syntax = "proto3";
  package comment_validate_test;
