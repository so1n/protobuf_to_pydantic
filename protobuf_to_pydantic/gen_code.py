...
from typing import Optional, Set, Tuple, Type, _GenericAlias  # type: ignore
    ...
    for key, value in model.__fields__.items():
        ...
        if value.type_.__module__ != "builtins":
            ...
            if isinstance(value_type, _GenericAlias):
                ...
                value_type_name = value_type.__str__()
                for sub_type in value_type.__args__:
                    if sub_type.__module__ == "builtins":
                        continue
                    _import_set, _depend_set, _class_set = _pydantic_model_to_py_code(sub_type)
                    if _import_set:
                        import_set.update(_import_set)
                    if _depend_set:
                        depend_set.update(_depend_set)
                    if _class_set:
                        depend_set.update(_class_set)
        ...
        field_str: str = re.sub(r"<class '(\w+)'>", r"\1", value.field_info.__repr__())
        class_str = class_str + " " * 4 + f"{key}: {value_type_name} = {field_str}\n"
    class_set.update({class_str})
    return import_set, depend_set, class_set
